...
def add_work(current_user: UserCreate, work: WorkDict):
    ...
    # Check if the user is authenticated
    if authenticate(current_user)["response"].lower() == "user authenticated":
        # find the user row from the database
...
def upload_image(image_data: bytes, current_user: UserCreate) -> str:
    global base_path
...
def save_video_file(video_file: UploadFile, current_user: UserCreate) -> str:
...
@app.get("/video_url/{name}/{password}/{title}")
def get_video_url(name: str, password: str, title: str):
    # Authenticate the user
    current_user = UserCreate(name=name, password=password)
    auth_response = authenticate(current_user)
    if auth_response["response"].lower() != "user authenticated":
        return {"response": "user not authenticated"}
